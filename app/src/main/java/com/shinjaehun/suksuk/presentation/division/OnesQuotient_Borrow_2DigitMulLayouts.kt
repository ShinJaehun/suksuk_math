...
val onesQuotientBorrow2DigitMulLayouts = listOf(
    ...
    DivisionStepUiLayout(
        ...
        cellConfigs = mergeWithAlwaysVisibleCells(
            mapOf(
                CellName.BorrowDividendTens to CellConfig(editable = true, highlight = Highlight.Editing),
                CellName.DividendTens to CellConfig(highlight = Highlight.Related, crossOutColor = CrossOutColor.Pending)
            )
        )
    ),
    // 3단계: 뺄셈(두 자리, 각 자리)
    DivisionStepUiLayout(
        ...
        cellConfigs = mergeWithAlwaysVisibleCells(
            mapOf(
                ...
                CellName.Borrowed10DividendOnes to CellConfig(editable = false, value = "10", highlight = Highlight.Related),
