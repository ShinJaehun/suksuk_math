...
class DivisionPhaseEvaluatorTestTwoByTwo {
    ...
    private val cases = listOf(
        // 68 ÷ 34
        PhaseInputTestCase(DivisionPhase.InputQuotient, DivisionCell.QuotientOnes, "2", info68x34, 0, listOf(), true),
        PhaseInputTestCase(DivisionPhase.InputMultiply1, DivisionCell.Multiply1Ones, "8", info68x34, 1, listOf("2"), true),
        PhaseInputTestCase(DivisionPhase.InputMultiply1, DivisionCell.Multiply1Tens, "6", info68x34, 2, listOf("2", "8"), true),
        PhaseInputTestCase(DivisionPhase.InputSubtract1, DivisionCell.Subtract1Ones, "0", info68x34, 3, listOf("2", "8", "6"), true),
        // 오답 케이스
        PhaseInputTestCase(DivisionPhase.InputQuotient, DivisionCell.QuotientOnes, "3", info68x34, 0, listOf(), false),
        PhaseInputTestCase(DivisionPhase.InputMultiply1, DivisionCell.Multiply1Ones, "7", info68x34, 1, listOf("2"), false),
        // 96 ÷ 12, 몫 8, 곱셈 Carry 발생 (16)
        PhaseInputTestCase(DivisionPhase.InputQuotient, DivisionCell.QuotientOnes, "8", info96x12, 0, listOf(), true),
        PhaseInputTestCase(DivisionPhase.InputMultiply1, DivisionCell.CarryDivisorTensM1, "1", info96x12, 1, listOf("8"), true),
        PhaseInputTestCase(DivisionPhase.InputMultiply1, DivisionCell.Multiply1Ones, "6", info96x12, 2, listOf("8", "1"), true),
        PhaseInputTestCase(DivisionPhase.InputMultiply1, DivisionCell.Multiply1Tens, "9", info96x12, 3, listOf("8", "1", "6"), true),
        PhaseInputTestCase(DivisionPhase.InputSubtract1, DivisionCell.Subtract1Ones, "0", info96x12, 4, listOf("8", "1", "6", "9"), true),
    )

    @Test
    fun test_PhaseEvaluator_batchCases() {
    ...
    fun multiply1TensCell_shows_question_when_editing_and_no_input() {
        ...
        assertEquals("?", uiState.cells[DivisionCell.Multiply1Tens]?.value)
    }
    ...
    fun borrowCell_shows_borrowed_value_and_crossout() {
        ...
        assertEquals(CrossOutType.Pending, uiState.cells[DivisionCell.DividendTens]?.crossOutType)
    }

    @Test
    fun isCorrect_returns_true_for_correct_input() {
        val phase = DivisionPhase.InputQuotient
        val cell = DivisionCell.QuotientOnes
        val dividend = 68
        val divisor = 34
        val userInput = "2"
